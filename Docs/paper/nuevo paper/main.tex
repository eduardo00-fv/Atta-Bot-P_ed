...
issued until the residual error falls below $3^\circ$.
...
feeding the same PID motion control, and the infrared proximity array with
equivalent placement and detection cones. The behaviors described above are
...
reported with the results: $2.5$ and $2.7 \cdot d_{robot}$ for the narrow
configurations and $5.0$ and $5.3 \cdot d_{robot}$ for the wide ones, that is,
about a quarter wider than nominal but with the two levels cleanly separated by a
factor of two. The simulated scenarios, generated programmatically, match the
...
repetitions each, and every run converged. Table~\ref{tab:sim} reports the
outcome.
